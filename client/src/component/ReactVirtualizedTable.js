...
function ReactVirtualizedTable(props) {
  let rows = props.state.coins.map((coin,index)=>{
    ...
    return {id:index+1, coin: coin.coin[0].toUpperCase()+coin.coin.slice(1), currentprice: current , initialprice: coin.price_per_unit , amount: coin.amount, netto: coin.amount*(current-coin.price_per_unit), total: coin.amount*current }
  })

  return (
    <Paper style={{ height: 400, width: '200%' }}>
